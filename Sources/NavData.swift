...
class NavDataParser {
    ...
    private func parseOption(_ tag: NavDataTag, data: Data, navData: inout NavData) {
        ...
        case .gps:
            parseGPSOption(data, navData: &navData)
        case .windSpeed:
    ...
    private func parseGPSOption(_ data: Data, navData: inout NavData) {
        guard data.count >= 32 else { return }
        
        var offset = 0
        navData.gpsLatitude = data.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: offset, as: Double.self) }
        offset += 8
        navData.gpsLongitude = data.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: offset, as: Double.self) }
        offset += 8
        navData.gpsAltitude = data.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: offset, as: Double.self) }
        offset += 8
        navData.gpsNumSatellites = Int(data.withUnsafeBytes { $0.loadUnaligned(fromByteOffset: offset, as: UInt32.self) })
    }
