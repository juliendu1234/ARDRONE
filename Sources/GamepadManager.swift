...
class GamepadManager {
    ...
    private let deadzone: Float = 0.20
    ...
    private let sensitivityStep: Float = 10.0
    private let minSensitivity: Float = 10.0
    private let maxSensitivity: Float = 100.0
    ...
    // MARK: - Controller Setup (MAPPING ANCIEN CODE)
    ...
    private func setupController(_ controller: GCController) {
        ...
        gamepad.leftTrigger.pressedChangedHandler = { [weak self] (button, value, pressed) in
            ...
            if pressed {
                print("L2 pressed - available for new function")
            }
        }
        
        // R2 - AVAILABLE (previously: increase sensitivity)
        // Sensitivity now controlled via UI sliders
        gamepad.rightTrigger.pressedChangedHandler = { [weak self] (button, value, pressed) in
            // TODO: Assign new function for R2
            if pressed {
                print("R2 pressed - available for new function")
            }
        }
    ...
    private func handleControllerInput(_ gamepad: GCExtendedGamepad) {
        ...
        // SDK-compliant: Pass stick values directly to drone
        // Sensitivity is now controlled by AT*CONFIG commands (euler_angle_max, control_vz_max, control_yaw)
        // No local multiplication needed
    ...
    private func showTemporaryNotification(title: String, message: String, duration: TimeInterval) {
        DispatchQueue.main.async {
            // Créer une fenêtre overlay temporaire
            ...
            notificationWindow.backgroundColor = NSColor(calibratedWhite: 0.1, alpha: 0.95)
            notificationWindow.isOpaque = false
            notificationWindow.level = .statusBar
            ...
            if let screen = NSScreen.main {
                ...
                let y = screenFrame.height - 200
                notificationWindow.setFrame(NSRect(x: x, y: y, width: 400, height: 120), display: true)
            }
            
            // Contenu
            let containerView = NSView(frame: notificationWindow.contentView!.bounds)
            ...
            titleLabel.frame = NSRect(x: 20, y: 70, width: 360, height: 30)
            ...
            messageLabel.frame = NSRect(x: 20, y: 20, width: 360, height: 40)
            messageLabel.maximumNumberOfLines = 2
            
            containerView.addSubview(titleLabel)
            containerView.addSubview(messageLabel)
            notificationWindow.contentView?.addSubview(containerView)
