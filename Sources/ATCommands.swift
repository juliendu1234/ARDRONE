...
class ATCommands {
    ...
    /// AFLIGHT command - Adaptive flight
    func aflight(flag: Int) -> String {
        return "AT*AFLIGHT=\(getNextSequence()),\(flag)\r"
    }
    
    /// PMODE command - Piloting mode
    func pmode(mode: Int) -> String {
        return "AT*PMODE=\(getNextSequence()),\(mode)\r"
    }
    ...
    /// Set max altitude (mm)
    func setMaxAltitude(_ altitude: Int) -> String {
        return config(key: "control:altitude_max", value: String(altitude))
    }
    
    /// Set max tilt angle (degrees * 1000)
    func setMaxTilt(_ angle: Int) -> String {
        return config(key: "control:euler_angle_max", value: String(angle))
    }
    
    /// Set max vertical speed (mm/s)
    func setMaxVerticalSpeed(_ speed: Int) -> String {
        return config(key: "control:control_vz_max", value: String(speed))
    }
    
    /// Set max yaw speed (°/s)
    func setMaxYawSpeed(_ speed: Float) -> String {
    ...
    /// Enable GPS
    func enableGPS(_ enabled: Bool) -> String {
        return config(key: "gps:gps_used", value: enabled ? "TRUE" : "FALSE")
    }
