...
class StatusWindowController: NSWindowController {
    ...
    private func createTelemetrySection() -> NSView {
        ...
        let colWidth: CGFloat = 240  // Increased from 230 for better spacing
        let rowHeight: CGFloat = 38   // Increased from 34 for better readability
    ...
    private func createMappingSection() -> NSView {
        ...
        let mappings: [(String, String, NSColor)] = [
            ...
            ("L1", "Définir Home", .systemOrange),
            ("R1", "Retour Home", .systemOrange),
            ("L2", "Disponible (TODO)", .systemGray),
            ("R2", "Disponible (TODO)", .systemGray),
        ...
        let cardHeight: CGFloat = 32  // Increased from 28
        let spacing: CGFloat = 8      // Increased from 6
        let startX: CGFloat = 20      // Increased from 16
        let startY: CGFloat = 52      // Increased from 48
        ...
        hint.font = NSFont.systemFont(ofSize: 11, weight: .medium)  // Increased size
        hint.textColor = .systemCyan  // Changed from gray for better visibility
    ...
    private func setupCallbacks() {
        ...
        droneController.onHomePointSet = { lat, lon in
            DispatchQueue.main.async {
                print("✅ Home point set in UI: \(lat), \(lon)")
            }
        }
